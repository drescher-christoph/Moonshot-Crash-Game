...
//   const [previousRoundState, setPreviousRoundState] = useState<number | null>(null)
...
//     if (roundIdNum !== null && previousRoundIdRef.current !== null && roundIdNum !== previousRoundIdRef.current) {
...
//       ) {
//         console.log("[v0] Detected LOCKED -> RESOLVED transition, starting animation")
//         setIsAnimating(true)
...
export function useGameState() {
  ...
  }) as { data: RoundData | undefined; refetch: any; isError: boolean; isLoading: boolean }
  ...
  const roundState = roundData ? (roundData[5]) : null
  const startTime = roundData ? Number(roundData[1]) : 0
  const lockTime = roundData ? Number(roundData[4]) : 0
  const crashTime = roundData ? Number(roundData[3]) : 0
  const crashMultiplier = roundData ? Number(roundData[2]) : 0
  ...
  useEffect(() => {
    if (roundState !== null) {
      ...
      // Normal state update when not animating and not in a special transition
        ...
        setDisplayState(Number(roundState))
      }

      setPreviousRoundState(Number(roundState))
    }
  }, [roundState, previousRoundState, isAnimating])
